...
function get_sort_list_for_left() {
	...
	echo '<li>
	<a href="'.Url::sort($value['sid']).'">'.$value['sortname'].'<span class="arrow"></span></a>
	</li>';
	}
}
...
function get_blogger_photo() {

	$blogger = get_blogger_all();

	if (!empty($blogger[1]['photo']['src'])) {
		return BLOG_URL.$blogger[1]['photo']['src'];
	} else {
		return false;
	}
...
function get_blogger_name() {
	$blogger = get_blogger_all();
	return $blogger[1]['name'];
}
?>
...
function editflg($logid,$author){
	$editflg = ROLE == ROLE_ADMIN || $author == UID ? ' <span>/</span> <a href="'.BLOG_URL.'admin/write_log.php?action=edit&gid='.$logid.'" target="_blank">编辑</a>' : '';
	echo $editflg;
}
?>
<?php
